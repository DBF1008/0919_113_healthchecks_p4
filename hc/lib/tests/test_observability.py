...
class ObservabilityTestCase(SimpleTestCase):
    def test_get_logger_logs_without_raising(self) -> None:
        # Note: the logger name must not start with "hc" -- the "hc" logger
        # has a database handler, and SimpleTestCase forbids db queries.
        logger = observability.get_logger("observability.test")
