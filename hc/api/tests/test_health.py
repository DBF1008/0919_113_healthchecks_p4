...
# The test runner sets EMAIL_HOST="example.org". Clear it at the class level
# so the SMTP check is "not_configured" by default; the SMTP-specific tests
# below set their own EMAIL_HOST values.
...
class HealthTestCase(BaseTestCase):
    ...
    def test_it_handles_db_error(self) -> None:
        cm = MagicMock()
        # Note: must be a django.db.Error subclass, so that the hc.logs
        # database handler (which logs the 503 response) can swallow it:
        cm.__enter__.side_effect = OperationalError("db is down")
